...
export default function AppointmentsListPage() {
  ...
  const visibleColumns = useMemo(() => meta.columns.filter((column) => !hiddenColumns.includes(column.key)), [hiddenColumns, meta.columns]);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-4">
        ...
        {loading ? <div className="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500">Loading appointments...</div> : null}
        ...
        {!loading && !error && rows.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-8 py-16 text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500">Book appointments against real service records, members, and CRM entities.</p>
          </div>
        ) : null}
          ...
          <div className="flex gap-3">
            {filterOpen ? (
              <FilterSidebar
                ...
                onApply={(filters) => { setSidebarFilters(filters); setPage(1); }}
                onClear={() => { setSidebarFilters({}); setPage(1); }}
              />
            ) : null}
            <div className="min-w-0 flex-1 space-y-3">
              <CRMTable
                ...
                onRowAction={async (actionKey, row) => {
                  ...
                  if (actionKey === "cancel") { await cancelAppointment(row.id); await load(); }
                }}
