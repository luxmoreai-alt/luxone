...
export default function BCCDropboxCard({ setting, onCreate, onUpdate, onRegenerate, onAddEmail, onVerifyEmail }: Props) {
  return (
    <CRMSectionCard
      ...
      action={
        !setting ? (
          <button type="button" onClick={onCreate} className="rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white">Create BCC Dropbox</button>
        ) : (
          <div className="flex gap-2">
            ...
            <button type="button" onClick={onRegenerate} className="rounded-md border border-blue-200 px-3 py-2 text-xs text-blue-700">Regenerate</button>
          </div>
        )
      }
    >
