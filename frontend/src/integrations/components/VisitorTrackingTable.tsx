...
export default function VisitorTrackingTable({ portals, events, onCreatePortal, onManagePortal, onDeactivatePortal, onViewCode, onConvertEvent }: Props) {
  return (
    <div className="space-y-4">
      <CRMSectionCard title="Visitor Portals" action={<button type="button" onClick={onCreatePortal} className="rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white">Create Portal</button>}>
        {!portals.length ? (
              ...
              <tbody>
                {events.map((event) => (
                  <tr key={event.id} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-3">
                      {!event.converted_to_lead ? <button type="button" onClick={() => onConvertEvent(event)} className="rounded-md border border-blue-200 px-3 py-1.5 text-xs text-blue-700">Convert to Lead</button> : null}
                    </td>
                  </tr>
                ))}
              </tbody>
