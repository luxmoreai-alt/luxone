...
export default function UnsubscribeLinksTable({ rows, onCreate, onEdit, onDelete }: Props) {
  return (
    <CRMSectionCard title="Unsubscribe Links" action={<button type="button" onClick={onCreate} className="rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white">Create Link</button>}>
      {!rows.length ? (
