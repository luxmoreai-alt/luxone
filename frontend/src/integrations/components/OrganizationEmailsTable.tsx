...
export default function OrganizationEmailsTable({ rows, onCreate, onEdit, onConfirm }: Props) {
  return (
    <CRMSectionCard
      ...
      action={<button type="button" onClick={onCreate} className="rounded-md bg-blue-600 px-3 py-2 text-xs font-medium text-white">Add Email</button>}
    >
      {!rows.length ? (
                  ...
                  <td className="px-3 py-3">
                    <div className="flex gap-2">
                      ...
                      {row.confirmation_status === "pending" ? (
                        <button type="button" onClick={() => onConfirm(row)} className="rounded-md border border-blue-200 px-3 py-1.5 text-xs text-blue-700">Confirm</button>
                      ) : null}
                    </div>
                  </td>
                </tr>
