  ...
  return (
    <Modal title="Send Email" onClose={onClose}>
      ...
      <div className="flex justify-end gap-2 border-t border-slate-200 px-6 py-3">
        ...
        <button type="button" onClick={handleSubmit} disabled={loading} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-60">
...
// Convert Modal
function ConvertModal({ leadId, onClose, onConverted }: {
  leadId: string;
  onClose: () => void;
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      const result = await apiRequest<{ account_id: number; contact_id: number; deal_id?: number | null }>(`/leads/${leadId}/convert/`, {
        method: "POST",
        body: JSON.stringify({
...
// Add Tags Modal
function AddTagsModal({ leadId, existingTags, onClose, onSaved }: { leadId: string; existingTags: string[]; onClose: () => void; onSaved: (tags: string[]) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (toAdd.length === 0) { setError("Add at least one tag"); return; }
    try {
      setLoading(true);
      ...
      <div className="px-6 py-4">
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {tags.map((t) => (
              <span key={t} className="inline-flex items-center gap-1 rounded-full bg-blue-100 px-2.5 py-0.5 text-xs text-blue-700">
                {t}
...
export default function LeadDetailPage() {
  ...
  const leadTags: string[] = (lead as { tags?: string[] }).tags ?? [];
    ...
    <>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="mt-2 space-y-1 text-sm text-green-700">
                <p>
                  Account:{" "}
                  <button type="button" className="underline font-medium hover:text-green-900" onClick={() => navigate(`/accounts/${convertedLinks.account_id}`)}>
                    View Account
                  </button>
                </p>
                <p>
                  Contact:{" "}
                  <button type="button" className="underline font-medium hover:text-green-900" onClick={() => navigate(`/contacts/${convertedLinks.contact_id}`)}>
                    View Contact
                  </button>
                </p>
                {convertedLinks.deal_id && (
                  <p>
                    Deal:{" "}
                    <button type="button" className="underline font-medium hover:text-green-900" onClick={() => navigate(`/deals/${convertedLinks.deal_id}`)}>
                      View Deal
                    </button>
                  </p>
                )}
      ...
      {activeModal === "convert" && (
        <ConvertModal
          ...
          onConverted={(result) => {
            ...
            void getLeadById(id!).then((updated) => { if (updated) setLead(updated); });
          }}
        />
      )}
      ...
      {activeModal === "add-tags" && (
        <AddTagsModal
          ...
          onClose={() => setActiveModal(null)}
          onSaved={(newTags) => setLead((prev) => prev ? { ...prev, tags: newTags } as LeadRecord : prev)}
        />
      )}
