...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--tenant-db",
            dest="tenant_db",
            help="Tenant DB name to audit. Defaults to current DB context.",
        )
        parser.add_argument(
            "--limit",
            type=int,
    ...
    def handle(self, *args, **options):
        tenant_db = options.get("tenant_db") or get_current_db_name() or "default"
        ...
        try:
            queryset = (
                SyncedEmailMessage.objects.using(tenant_db)
            ...
            self.stdout.write(self.style.SUCCESS(f"Email audit for DB: {tenant_db}"))
